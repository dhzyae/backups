<template>
  <div class="app-container">
    ...
    <!-- 工具条 -->
    <div class="tools-div">
      <el-button type="success" icon="el-icon-plus" size="mini" :disabled="$hasBP('bnt.processTemplate.templateSet') === false" @click="add()">添加审批模板</el-button>
    </div>
    ...
    >
      ...
      <el-table-column label="操作" width="250" align="center">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.status == 0" type="text" size="mini" :disabled="$hasBP('bnt.processTemplate.templateSet') === false" @click="edit(scope.row.id)">修改审批设置</el-button>
          <el-button v-if="scope.row.status == 0" type="text" size="mini" :disabled="$hasBP('bnt.processTemplate.remove') === false" @click="removeDataById(scope.row.id)">删除</el-button>

          <el-button v-if="scope.row.status == 0" type="text" size="mini" :disabled="$hasBP('bnt.processTemplate.publish') === false" @click="publish(scope.row.id)">发布</el-button>
        </template>
      </el-table-column>
    </el-table>
